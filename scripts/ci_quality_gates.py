from __future__ import annotations

import os
import shutil
import subprocess
import sys
from dataclasses import dataclass
from pathlib import Path


ROOT = Path(__file__).resolve().parents[1]
FRONTEND = ROOT / "frontend"
PYTHON = sys.executable
NPM = shutil.which("npm") or "npm"


@dataclass(frozen=True)
class QualityGate:
    label: str
    command: list[str]
    cwd: Path = ROOT


QUALITY_GATES = (
    QualityGate(
        label="python-format-lint",
        command=[PYTHON, "scripts/python_static_quality.py"],
    ),
    QualityGate(
        label="python-type-check",
        command=[
            PYTHON,
            "-m",
            "mypy",
            "--no-sqlite-cache",
            "--ignore-missing-imports",
            "--follow-imports=skip",
            "--allow-untyped-defs",
            "--no-strict-optional",
            "--no-warn-unused-ignores",
            "backend/app/domain/models.py",
            "backend/app/api/v2.py",
            "backend/app/trading_policy/models.py",
            "backend/app/gates/models.py",
            "scripts/ci_quality_gates.py",
            "scripts/python_static_quality.py",
        ],
    ),
    QualityGate(
        label="pytest",
        command=[PYTHON, "-m", "pytest", "backend/tests", "-q"],
    ),
    QualityGate(
        label="regime-focused-tests",
        command=[
            PYTHON,
            "-m",
            "pytest",
            "backend/tests/regime",
            "-q",
            "--cov=backend/app/algorithms/regime",
            "--cov-branch",
            "--cov-fail-under=54",
        ],
    ),
    QualityGate(
        label="meta-strategy-dedicated-tests",
        command=[
            PYTHON,
            "-m",
            "pytest",
            "backend/tests/meta_strategy",
            "-q",
            "--cov=backend/app/algorithms/meta_strategy",
            "--cov-branch",
            "--cov-fail-under=85",
        ],
    ),
    QualityGate(
        label="session-dedicated-tests",
        command=[
            PYTHON,
            "-m",
            "pytest",
            "backend/tests/test_session_calendar.py",
            "backend/tests/test_session_data_quality.py",
            "backend/tests/test_session_opening_range.py",
            "backend/tests/test_session_vwap.py",
            "backend/tests/test_session_baselines.py",
            "backend/tests/test_session_volatility.py",
            "backend/tests/test_session_volume.py",
            "backend/tests/test_session_liquidity.py",
            "backend/tests/test_session_structure.py",
            "backend/tests/test_session_classifier.py",
            "backend/tests/test_session_transitions.py",
            "backend/tests/test_session_profiles.py",
            "backend/tests/test_session_cost_gate.py",
            "backend/tests/test_session_runtime_parity.py",
            "backend/tests/test_session_persistence.py",
            "backend/tests/test_session_frontend_contract.py",
            "backend/tests/test_session_research_calibration.py",
            "backend/tests/test_session_step19_rollout.py",
            "backend/tests/test_session_final_acceptance.py",
            "-q",
        ],
    ),
    QualityGate(
        label="typescript-type-check",
        command=[NPM, "run", "typecheck"],
        cwd=FRONTEND,
    ),
    QualityGate(
        label="frontend-tests",
        command=[NPM, "test"],
        cwd=FRONTEND,
    ),
    QualityGate(
        label="frontend-build",
        command=[NPM, "run", "build:ci"],
        cwd=FRONTEND,
    ),
    QualityGate(
        label="database-migration-test",
        command=[PYTHON, "-m", "pytest", "backend/tests/test_snapshot_persistence_migrations.py", "-q"],
    ),
    QualityGate(
        label="deterministic-replay-test",
        command=[
            PYTHON,
            "-m",
            "pytest",
            "backend/tests/test_event_driven_replay_engine.py",
            "backend/tests/test_v2_e2e_replay_fixtures.py",
            "-q",
        ],
    ),
    QualityGate(
        label="schema-compatibility-test",
        command=[
            PYTHON,
            "-m",
            "pytest",
            "backend/tests/test_domain_models_v2.py",
            "backend/tests/test_trading_settings_schema.py",
            "backend/tests/test_candidate_meta_features.py",
            "backend/tests/test_safe_ml_inference_modes.py",
            "backend/tests/test_frontend_v2_presentation.py",
            "-q",
        ],
    ),
    QualityGate(
        label="safety-critical-regression-tests",
        command=[
            PYTHON,
            "-m",
            "pytest",
            "backend/tests/test_phase12_comprehensive.py",
            "backend/tests/test_point_in_time_feature_engine.py",
            "backend/tests/test_decision_snapshot_v2_archive.py",
            "backend/tests/test_candidate_meta_features.py",
            "backend/tests/test_safe_ml_inference_modes.py",
            "backend/tests/test_meta_strategy_step6_architecture_isolation.py",
            "backend/tests/test_meta_strategy_step42_frontend_boundary.py",
            "backend/tests/test_session_classifier.py",
            "backend/tests/test_session_runtime_parity.py",
            "backend/tests/test_session_frontend_contract.py",
            "backend/tests/test_session_research_calibration.py",
            "backend/tests/test_session_step19_rollout.py",
            "backend/tests/test_session_final_acceptance.py",
            "backend/tests/test_global_gate_engine.py",
            "backend/tests/test_wca_step19_comprehensive.py",
            "backend/tests/test_wca_step21_final_acceptance.py",
            "backend/tests/test_regime_final_acceptance.py",
            "backend/tests/test_regime_phase17_rollout.py",
            "backend/tests/regime",
            "backend/tests/test_weighted_voting_final_acceptance.py",
            "backend/tests/test_weighted_voting_step32_comprehensive.py",
            "backend/tests/test_event_driven_replay_engine.py",
            "-q",
        ],
    ),
    QualityGate(
        label="wca-safety-critical-tests",
        command=[
            PYTHON,
            "-m",
            "pytest",
            "backend/tests/test_wca_step1_backend_structure.py",
            "backend/tests/test_wca_step2_configuration_system.py",
            "backend/tests/test_wca_step3_strategy_catalog.py",
            "backend/tests/test_wca_step3_primary_strategy_validation.py",
            "backend/tests/test_wca_step4_modifiers_hard_filters.py",
            "backend/tests/test_wca_step4_strategy_isolation.py",
            "backend/tests/test_wca_step5_confidence_calibration.py",
            "backend/tests/test_wca_step5_production_pipeline.py",
            "backend/tests/test_wca_step6_inventory_persistence.py",
            "backend/tests/test_wca_step6_performance_weights.py",
            "backend/tests/test_wca_step7_background_runtime.py",
            "backend/tests/test_wca_step8_dynamic_profile.py",
            "backend/tests/test_wca_step8_research_worker.py",
            "backend/tests/test_wca_step9_global_risk_final_validation.py",
            "backend/tests/test_wca_step10_local_gates.py",
            "backend/tests/test_wca_step10_paper_broker_outbox.py",
            "backend/tests/test_wca_step11_position_management_reconciliation.py",
            "backend/tests/test_wca_step11_sizing.py",
            "backend/tests/test_wca_step12_calibration_weights.py",
            "backend/tests/test_wca_step12_global_gate_engine.py",
            "backend/tests/test_wca_step13_dynamic_cost_latency.py",
            "backend/tests/test_wca_step13_ml_forecast_decoupling.py",
            "backend/tests/test_wca_step14_15_backend_backtest.py",
            "backend/tests/test_wca_step15_api_frontend_control_surface.py",
            "backend/tests/test_wca_step16_safety_critical_ci.py",
            "backend/tests/test_wca_step17_persistence.py",
            "backend/tests/test_wca_step18_frontend_presentation.py",
            "backend/tests/test_wca_step19_comprehensive.py",
            "backend/tests/test_wca_step20_rollout.py",
            "backend/tests/test_wca_step21_final_acceptance.py",
            "backend/tests/test_wca_broker_reconciliation.py",
            "backend/tests/test_wca_modifier_inventory.py",
            "backend/tests/test_wca_paper_execution_pipeline.py",
            "backend/tests/test_wca_paper_stability_validation.py",
            "backend/tests/test_wca_shadow_comparison_evidence.py",
            "-q",
        ],
    ),
    QualityGate(
        label="regime-final-acceptance",
        command=[
            PYTHON,
            "-m",
            "pytest",
            "backend/tests/test_regime_final_acceptance.py",
            "backend/tests/test_regime_phase13_backtest_api.py",
            "backend/tests/test_regime_phase14_persistence.py",
            "backend/tests/test_regime_phase17_rollout.py",
            "backend/tests/regime/test_step10_paper_readiness_rollout.py",
            "backend/tests/test_global_account_risk_state.py",
            "backend/tests/test_global_portfolio_risk_manager_phase12.py",
            "-q",
        ],
    ),
    QualityGate(
        label="weighted-voting-final-acceptance",
        command=[
            PYTHON,
            "-m",
            "pytest",
            "backend/tests/test_weighted_voting_final_acceptance.py",
            "backend/tests/test_weighted_voting_ml_decoupling.py",
            "backend/tests/test_weighted_voting_algorithm_isolation.py",
            "backend/tests/test_weighted_voting_aggregation.py",
            "backend/tests/test_weighted_voting_decision_gates.py",
            "backend/tests/test_weighted_voting_settings.py",
            "backend/tests/test_weighted_voting_backtest_engine.py",
            "backend/tests/test_weighted_voting_walk_forward.py",
            "backend/tests/test_weighted_voting_paper_order_gateway.py",
            "backend/tests/test_weighted_voting_step32_comprehensive.py",
            "-q",
        ],
    ),
    QualityGate(
        label="v2-completion-readiness",
        command=[PYTHON, "scripts/v2_readiness_gate.py"],
    ),
)


def run_gate(gate: QualityGate) -> int:
    env = os.environ.copy()
    env.setdefault("PYTHONDONTWRITEBYTECODE", "1")
    env.setdefault("PYTHONPATH", str(ROOT))

    print(f"::group::{gate.label}")
    print(f"$ {' '.join(gate.command)}")
    result = subprocess.run(gate.command, cwd=gate.cwd, env=env, check=False)
    print("::endgroup::")
    return result.returncode


def main() -> int:
    failures: list[str] = []
    for gate in QUALITY_GATES:
        returncode = run_gate(gate)
        if returncode:
            failures.append(f"{gate.label} exited with {returncode}")

    if failures:
        print("Quality gates failed:")
        for failure in failures:
            print(f"  - {failure}")
        return 1

    print("All quality gates passed.")
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
