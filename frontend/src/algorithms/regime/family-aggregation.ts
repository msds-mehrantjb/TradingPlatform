...
export function aggregateRegimeStrategyScores(strategyOutputs: RegimeSelectedStrategy[]): RegimeAggregationResult {
  const directionalOutputs = strategyOutputs.filter((output) => output.role === "directional" && output.eligible);
  const directionalActive = directionalOutputs.filter((output) => output.signal === "buy" || output.signal === "sell");
  ...
  directionalActive.forEach((output) => {
    ...
    const cappedContribution = Math.min(
      ...
      Math.max(0, clampNumber(output.confidence, 0, 1) * Math.max(0, output.effectiveWeight || output.effective_weight || 0)),
    );
  ...
  const familyScores = Array.from(familyAccumulators.entries()).map(([family, accumulator]): RegimeFamilyScore => {
    const familyTotal = accumulator.buy + accumulator.sell;
    const familyScale = familyTotal > REGIME_MAX_FAMILY_CONTRIBUTION ? REGIME_MAX_FAMILY_CONTRIBUTION / familyTotal : 1;
    return {
      family,
      buyScore: roundNumber(accumulator.buy * familyScale, 4),
      sellScore: roundNumber(accumulator.sell * familyScale, 4),
  ...
  const abstentionRate = directionalOutputs.length ? roundNumber((directionalOutputs.length - directionalActive.length) / directionalOutputs.length, 4) : 1;
