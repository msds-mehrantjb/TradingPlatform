...
import { buildRegimeDecisionEvidence } from "./decision-evidence.ts";
import { regimeTradeBlockers } from "./decision-gates.ts";
...
export function calculateRegimeDecision(input: RegimeDecisionInput): RegimeDecisionOutput {
  ...
  result.decisionSnapshot = buildRegimeDecisionEvidence(result, mlSnapshot, input);
