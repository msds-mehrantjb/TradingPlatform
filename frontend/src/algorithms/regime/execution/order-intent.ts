...
import { generateRegimeOrderIntentIdempotencyKey } from "./idempotency.ts";
import { regimeOrderIntentFailureReasons, shortSaleGateReasons, validateRegimeOrderIntent, validateRegimeTargetOrder } from "./order-validation.ts";
...
export function buildRegimeTargetOrder(
  ...
  const validationErrors = intent ? validateRegimeOrderIntent(intent) : [];
  const failedGates = intent
    ? validationErrors
    : regimeOrderIntentFailureReasons({
        result,
        signalDirection,
        positionEffect,
        requestedQuantity: sizing.finalQuantity,
        expectedEntryPrice: triggerPrice,
        protectiveStopPrice: stopPrice,
        targetPrice,
        options,
      });
  const orderType = intent ? `${intent.signal} stop-limit` : "No order";
  const target: RegimeTargetOrder = {
    eligible: intent !== null && validationErrors.length === 0,
    ...
    orderIntent: validationErrors.length === 0 ? intent : null,
  ...
  const finalValidationErrors = validateRegimeTargetOrder(target);
  return finalValidationErrors.length === 0 || target.orderIntent === null
    ? target
    : { ...target, eligible: false, orderIntent: null, failedGates: finalValidationErrors };
}
