...
# Legacy weight records naming these strategies must not be imported as voting weight.
# They are catalogued shadow modules now, so they hold a slot at exactly zero rather
# than being absent.
LEGACY_NON_VOTING_STRATEGY_IDS = ("S1", "S3", "S4", "S8")
...
class WeightedVotingMigrationTest(unittest.TestCase):
    def test_migration_preserves_compatible_settings_and_archives_untrusted_weights(self) -> None:
        ...
        self.assertTrue(set(WEIGHTED_VOTING_ACTIVE_STRATEGY_IDS) <= set(active_weights["strategy_weights"]))
        ...
        self.assertTrue(all(active_weights["strategy_weights"].get(strategy_id, 0.0) == 0.0 for strategy_id in LEGACY_NON_VOTING_STRATEGY_IDS))
    ...
    def test_trustworthy_weight_history_is_preserved_but_not_activated(self) -> None:
        ...
        self.assertTrue(set(WEIGHTED_VOTING_ACTIVE_STRATEGY_IDS) <= set(active))
        self.assertTrue(all(active[strategy_id] == 0.25 for strategy_id in WEIGHTED_VOTING_ACTIVE_STRATEGY_IDS))
        self.assertTrue(all(active.get(strategy_id, 0.0) == 0.0 for strategy_id in LEGACY_NON_VOTING_STRATEGY_IDS))
