...
class WeightedVotingSettingsTest(unittest.TestCase):
    def test_baseline_configuration_is_versioned_and_owned_by_weighted_voting(self) -> None:
        ...
        self.assertEqual(config.paper_execution_mode, "LOCAL_PAPER")
        self.assertEqual(baseline["weighted_voting.paper.execution_mode"], "LOCAL_PAPER")
        self.assertEqual(baseline["paperExecutionMode"], "LOCAL_PAPER")
        self.assertEqual(config.local_paper_initial_capital, 100000.0)
        self.assertEqual(baseline["weighted_voting.local_paper.initial_capital"], config.local_paper_initial_capital)
        self.assertEqual(baseline["weighted_voting.local_paper.commission"], config.fee_per_share)
        self.assertEqual(baseline["weighted_voting.local_paper.slippage"], config.entry_slippage_per_share)
        self.assertEqual(baseline["weighted_voting.local_paper.allow_shorting"], config.local_paper_allow_shorting)
        self.assertEqual(baseline["weighted_voting.local_paper.partial_fill_mode"], config.local_paper_partial_fill_mode)
        self.assertEqual(baseline["localPaper"]["initialCapital"], config.local_paper_initial_capital)
        self.assertEqual(baseline["localPaper"]["executionMode"], "LOCAL_PAPER")
        self.assertEqual(baseline["localPaper"]["owner"], "weighted_voting")
        self.assertFalse(baseline["localPaper"]["allowShorting"])
        self.assertEqual(baseline["localPaper"]["partialFillMode"], "DETERMINISTIC_LIQUIDITY")
        self.assertEqual(baseline["localPaper"]["executionCosts"]["slippagePerShare"], config.entry_slippage_per_share)
        self.assertEqual(baseline["localPaper"]["executionCosts"]["commission"], config.fee_per_share)
        self.assertEqual(baseline["localPaper"]["executionCosts"]["buySlippagePerShare"], config.entry_slippage_per_share)
        self.assertEqual(baseline["localPaper"]["executionCosts"]["commissionPerShare"], config.fee_per_share)
        self.assertEqual(baseline["localPaper"]["executionCosts"]["regulatoryFeePerShare"], config.regulatory_fee_per_share)
        self.assertEqual(baseline["localPaper"]["executionCosts"]["spreadImpactPerShare"], config.spread_impact_per_share)
    ...
    def test_dynamic_settings_begin_from_weighted_voting_baseline(self) -> None:
        ...
        self.assertEqual(defaults.paper_execution_mode, config.paper_execution_mode)
        self.assertEqual(defaults.local_paper_initial_capital, config.local_paper_initial_capital)
        self.assertEqual(defaults.local_paper_buy_slippage_per_share, config.entry_slippage_per_share)
        self.assertEqual(defaults.local_paper_sell_slippage_per_share, config.exit_slippage_per_share)
        self.assertEqual(defaults.local_paper_commission_per_share, config.fee_per_share)
        self.assertEqual(defaults.local_paper_partial_fill_mode, config.local_paper_partial_fill_mode)
        ...
        self.assertEqual(effective.paper_execution_mode, config.paper_execution_mode)
        self.assertEqual(effective.local_paper_initial_capital, config.local_paper_initial_capital)
        self.assertEqual(effective.local_paper_commission_per_share, config.fee_per_share)
