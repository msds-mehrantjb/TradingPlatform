...
class VotingEnsembleAutomaticPaperExecutionTest(unittest.TestCase):
    ...
    def test_paper_inventory_snapshot_reports_backend_orders_fills_and_positions(self) -> None:
        ...
        self.assertEqual(inventory["executionMode"], "LOCAL_PAPER")
        self.assertEqual(inventory["sourceAuthority"], "voting_ensemble_local_paper_account")
        self.assertEqual(inventory["localPaperAccount"]["accountId"], VOTING_ENSEMBLE_LOCAL_ACCOUNT_ID)
        self.assertEqual(inventory["initialCash"], 100000.0)
        self.assertEqual(inventory["cash"], 99700.0)
        self.assertEqual(inventory["equity"], 100000.0)
        self.assertEqual(inventory["buyingPower"], 99700.0)
        self.assertEqual(inventory["realizedPnl"], 0.0)
        self.assertEqual(inventory["realizedPnlToday"], 0.0)
        self.assertEqual(inventory["unrealizedPnl"], 0.0)
        self.assertEqual(inventory["dailyNetPnl"], 0.0)
        self.assertEqual(inventory["localPositions"], inventory["positions"])
        self.assertEqual(inventory["localOrders"], inventory["orders"])
        self.assertEqual(len(inventory["openOrders"]), 2)
        self.assertEqual(inventory["recentFills"][0]["clientOrderId"], inventory["fills"][0]["clientOrderId"])
        self.assertEqual(inventory["closedTrades"], [])
        self.assertEqual(inventory["grossExposure"], 300.0)
        self.assertEqual(inventory["netExposure"], 300.0)
        self.assertEqual(inventory["openRisk"], 3.0)
        self.assertEqual(inventory["drawdown"], 0.0)
