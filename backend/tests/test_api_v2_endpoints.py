    ...
    def test_algorithm_inventory_endpoints_share_v2_module_shape(self) -> None:
        ...
        for path_id, (algorithm_id, populated_group) in expected.items():
            with self.subTest(path_id=path_id):
                ...
                self.assertTrue({"directional", "context", "regime", "safety", "aggregator"}.issubset(set(body["modules"])))
        ...
        expected_modules = {
            "voting-ensemble": {
                ...
                "tradingSettings": ["trading_settings_resolver"],
                "riskBudget": ["risk_budget"],
                "orderPlanner": ["order_planner"],
                "executionAdapter": ["execution_adapter"],
                "backtestReplay": ["backtest_replay_adapter"],
            ...
            "regime": {
                ...
                "context": ["volume_confirmation", "adx_trend_strength"],
                "regime": ["vwap_position", "atr_volatility_regime"],
                "safety": [
                    "cash_avoid_filter",
