...
class RouterTest(unittest.TestCase):
    def test_risk_off_regime_skips_directional_strategies(self):
        routing = route_regime_strategies(
            snapshot("up"),
            classification(raw_regime="event_risk", event_risk="blackout"),
            {"noNewEntries": True, "allowedStrategyFamilies": (), "disabledStrategyFamilies": ("trend", "breakout")},
        )

        self.assertEqual(routing["selectedStrategyIds"], ())
        self.assertTrue(routing["profileRouting"]["noNewEntries"])
        self.assertIn("trend", routing["profileRouting"]["disabledStrategyFamilies"])
