"""Weighted Voting V2 isolated backend package."""

from backend.app.algorithms.weighted_voting.catalog import (
    WEIGHTED_VOTING_BASELINE_STRATEGY_WEIGHT,
    WEIGHTED_VOTING_MAXIMUM_STRATEGY_WEIGHT,
    WEIGHTED_VOTING_MINIMUM_STRATEGY_WEIGHT,
    WEIGHTED_VOTING_MODULE_INVENTORY,
    WeightedVotingDedicatedStrategyInventoryItem,
    WeightedVotingModuleInventory,
    WeightedVotingModuleLifecycleStatus,
    WeightedVotingModuleStatus,
    weighted_voting_dedicated_strategy_inventory,
    weighted_voting_enabled_strategy_catalog,
    weighted_voting_module_inventory,
)
from backend.app.algorithms.weighted_voting.architecture import (
    WEIGHTED_VOTING_ARCHITECTURE_VERSION,
    WEIGHTED_VOTING_AUTHORITATIVE_PACKAGE,
    WEIGHTED_VOTING_BACKTEST_KERNEL,
    WEIGHTED_VOTING_CAPITAL_PARTITION_ID as WEIGHTED_VOTING_ARCHITECTURE_CAPITAL_PARTITION_ID,
    WEIGHTED_VOTING_DECISION_KERNEL,
    WEIGHTED_VOTING_FILESYSTEM_ROOT,
    WEIGHTED_VOTING_OWNED_MUTABLE_DOMAINS,
    WEIGHTED_VOTING_PIPELINE_BOUNDARIES,
    WEIGHTED_VOTING_SHARED_READ_ONLY_PORTS,
    WEIGHTED_VOTING_STORAGE_NAMESPACE,
    WeightedVotingArchitectureContract,
    WeightedVotingPipelineBoundary,
    WeightedVotingReadOnlyPort,
    weighted_voting_architecture_contract,
    weighted_voting_architecture_status,
)
from backend.app.algorithms.weighted_voting.alpaca_paper_broker import (
    WEIGHTED_VOTING_ALPACA_PAPER_BROKER_VERSION,
    WeightedVotingAlpacaPaperBroker,
    WeightedVotingAlpacaPaperBrokerConfigurationError,
    WeightedVotingUnavailablePaperBroker,
    build_weighted_voting_paper_gateway_dependencies,
)
from backend.app.algorithms.weighted_voting.local_paper_broker import (
    WEIGHTED_VOTING_LOCAL_PAPER_BROKER_VERSION,
    WEIGHTED_VOTING_LOCAL_PAPER_NAMESPACE,
    WeightedVotingLocalPaperAccount,
    WeightedVotingLocalPaperBroker,
    WeightedVotingLocalPaperRiskPort,
    WeightedVotingLocalPaperRiskService,
    build_weighted_voting_local_paper_gateway_dependencies,
)
from backend.app.algorithms.weighted_voting.config import (
    WEIGHTED_VOTING_BASELINE_CONFIGURATION_KEY,
    WEIGHTED_VOTING_CONFIG_VERSION,
    WEIGHTED_VOTING_CONFIGURATION_NAMESPACE,
    WeightedVotingConfig,
)
from backend.app.algorithms.weighted_voting.broker_reconciliation import (
    WEIGHTED_VOTING_BROKER_RECONCILIATION_NAMESPACE,
    WEIGHTED_VOTING_BROKER_RECONCILIATION_VERSION,
    WeightedVotingBrokerFillObservation,
    WeightedVotingBrokerOrderObservation,
    WeightedVotingBrokerPositionObservation,
    WeightedVotingBrokerReconciliationDiscrepancy,
    WeightedVotingBrokerReconciliationResult,
    WeightedVotingReconciliationDiscrepancySeverity,
    reconcile_weighted_voting_broker_observations,
    reconciliation_status,
)
from backend.app.algorithms.weighted_voting.decision_kernel import (
    WEIGHTED_VOTING_DECISION_KERNEL_VERSION,
    WeightedVotingDecisionKernel,
    WeightedVotingDecisionResult,
    decision_kernel_status,
)
from backend.app.algorithms.weighted_voting.entry_policy import (
    WEIGHTED_VOTING_ENTRY_POLICY_FRONTEND_MODE,
    WEIGHTED_VOTING_ENTRY_POLICY_VERSION,
    WeightedEntryPolicyResult,
    WeightedEntryTiming,
    WeightedEntryType,
    entry_policy_status,
    evaluate_entry_policy,
)
from backend.app.algorithms.weighted_voting.exit_policy import (
    WEIGHTED_VOTING_EXIT_POLICY_VERSION,
    WeightedExitAction,
    WeightedVotingExitDecision,
    WeightedVotingExitInputs,
    WeightedVotingExitLifecycleState,
    evaluate_exit_lifecycle,
    exit_policy_status,
    open_exit_lifecycle,
)
from backend.app.algorithms.weighted_voting.execution_gateway import (
    WEIGHTED_VOTING_BROKER_CONNECTION_BOUNDARY,
    WEIGHTED_VOTING_EXECUTION_GATEWAY_VERSION,
    WEIGHTED_VOTING_EXECUTION_NAMESPACE,
    WEIGHTED_VOTING_EXECUTION_OWNERSHIP,
    WeightedVotingBrokerCommand,
    WeightedVotingExecutionLifecycleRecord,
    WeightedVotingExecutionQueueItem,
    WeightedVotingExecutionReconciliation,
    WeightedVotingExecutionRejection,
    WeightedVotingExecutionStatus,
    WeightedVotingExecutionSubmission,
    WeightedVotingFillAttribution,
    WeightedVotingPositionAttribution,
    build_weighted_voting_broker_command,
    deterministic_weighted_voting_client_order_id,
    enqueue_weighted_voting_execution_order,
    execution_gateway_status,
    persist_weighted_voting_broker_command,
    reconcile_weighted_voting_broker_result,
    record_weighted_voting_fill,
    record_weighted_voting_rejection,
    record_weighted_voting_submission,
    submit_queued_weighted_voting_paper_order,
    submit_weighted_voting_paper_order,
)
from backend.app.algorithms.weighted_voting.final_acceptance import (
    WEIGHTED_VOTING_FINAL_ORDER_ACCEPTANCE_VERSION,
    WeightedVotingFinalOrderAcceptanceCheck,
    WeightedVotingFinalOrderAcceptanceInputs,
    WeightedVotingFinalOrderAcceptanceResult,
    evaluate_weighted_voting_final_order_acceptance,
)
from backend.app.algorithms.weighted_voting.global_interface import (
    WEIGHTED_VOTING_ALLOWED_GLOBAL_ACTIONS,
    WEIGHTED_VOTING_CAPITAL_PARTITION_ID,
    WEIGHTED_VOTING_GLOBAL_RISK_CONTRACT_VERSION,
    WEIGHTED_VOTING_GLOBAL_IMMUTABILITY_CHECKS,
    WEIGHTED_VOTING_GLOBAL_INTERFACE_VERSION,
    WeightedVotingGlobalRiskRequest,
    WeightedVotingGlobalRiskResponse,
    WeightedVotingStaticCentralGlobalRiskService,
    WeightedVotingUnavailableCentralGlobalRiskService,
    build_weighted_voting_global_risk_request,
    build_global_order_proposal_from_weighted_voting_proposal,
    global_gate_response_from_weighted_voting_risk,
    global_interface_status,
    validate_weighted_voting_global_risk_response,
)
from backend.app.algorithms.weighted_voting.identity import (
    WEIGHTED_VOTING_ACTIVE_WEIGHT_VERSION,
    WEIGHTED_VOTING_ALGORITHM_ID,
    WEIGHTED_VOTING_API_NAMESPACE,
    WEIGHTED_VOTING_API_TAG,
    WEIGHTED_VOTING_API_VERSION,
    WEIGHTED_VOTING_CONFIGURATION_VERSION,
    WEIGHTED_VOTING_ERROR_CODE_PREFIX,
    WEIGHTED_VOTING_REASON_CODE_PREFIX,
    WEIGHTED_VOTING_STRATEGY_VERSION,
    WeightedVotingServiceBoundary,
    weighted_voting_reason_code,
    weighted_voting_service_boundary,
)
from backend.app.algorithms.weighted_voting.inventory import (
    WEIGHTED_VOTING_INVENTORY_NAMESPACE,
    WEIGHTED_VOTING_INVENTORY_VERSION,
    WeightedVotingCapitalPartition,
    WeightedVotingDailyLedger,
    WeightedVotingInventoryEvent,
    WeightedVotingInventoryEventType,
    WeightedVotingInventoryRepository,
    WeightedVotingInventorySnapshot,
    WeightedVotingPendingOrder,
    WeightedVotingPosition,
    WeightedVotingRiskUsage,
    inventory_status,
)
from backend.app.algorithms.weighted_voting.market_snapshot import WEIGHTED_VOTING_MARKET_SNAPSHOT_ADAPTER_VERSION, build_weighted_voting_market_snapshot
from backend.app.algorithms.weighted_voting.order_proposal import (
    WEIGHTED_VOTING_ORDER_PROPOSAL_OWNERSHIP,
    WEIGHTED_VOTING_ORDER_PROPOSAL_VERSION,
    WeightedVotingOrderProposal,
    build_weighted_voting_order_proposal,
)
from backend.app.algorithms.weighted_voting.position_trade_state import (
    WEIGHTED_VOTING_POSITION_TRADE_STATE_NAMESPACE,
    WEIGHTED_VOTING_POSITION_TRADE_STATE_VERSION,
    WeightedVotingOrderLifecycle,
    WeightedVotingOrderState,
    WeightedVotingPositionLifecycle,
    WeightedVotingPositionTradeState,
    assert_weighted_voting_position_ownership,
    close_weighted_voting_position,
    create_weighted_voting_order_state,
    mark_weighted_voting_order_cancelled,
    mark_weighted_voting_order_filled,
    mark_weighted_voting_order_open,
    mark_weighted_voting_order_partial_fill,
    mark_weighted_voting_order_rejected,
    open_weighted_voting_position_from_order,
    persist_weighted_voting_order_state,
    persist_weighted_voting_position_state,
    position_trade_state_status,
    update_weighted_voting_position_mark,
)
from backend.app.algorithms.weighted_voting.position_manager import (
    WEIGHTED_VOTING_POSITION_MANAGER_NAMESPACE,
    WEIGHTED_VOTING_POSITION_MANAGER_VERSION,
    WeightedVotingAuthoritativeTradeRecord,
    WeightedVotingExitInstruction,
    WeightedVotingManagedOrderType,
    WeightedVotingPositionManagerService,
    WeightedVotingProtectiveInstruction,
    assert_weighted_voting_position_manager_ownership,
    position_manager_status,
)
from backend.app.algorithms.weighted_voting.performance_tracker import (
    WEIGHTED_VOTING_PERFORMANCE_TRACKER_NAMESPACE,
    WEIGHTED_VOTING_PERFORMANCE_TRACKER_VERSION,
    WeightedVotingAlgorithmPerformance,
    WeightedVotingMarketConditionPerformance,
    WeightedVotingPerformanceBucket,
    WeightedVotingPerformanceReport,
    WeightedVotingPerformanceSnapshot,
    WeightedVotingSignalObservation,
    WeightedVotingStrategyPerformance,
    WeightedVotingTrackedTrade,
    WeightedVotingWeightVersionPerformance,
    WeightedVotingWeightVersionSnapshot,
    build_weighted_voting_performance_report,
    performance_tracker_status,
    persist_weighted_voting_performance_report,
)
from backend.app.algorithms.weighted_voting.persistence import (
    WEIGHTED_VOTING_ARTIFACT_CATEGORIES,
    WEIGHTED_VOTING_COLLECTION_ALIASES,
    WEIGHTED_VOTING_PERSISTENCE_VERSION,
    WEIGHTED_VOTING_REQUIRED_COLLECTIONS,
    WEIGHTED_VOTING_REQUIRED_RECORD_FIELDS,
    WEIGHTED_VOTING_SETTINGS_KEY,
    WEIGHTED_VOTING_STORAGE_ROOT,
    WeightedVotingArtifactMetadata,
    WeightedVotingFilesystemStateStore,
    canonical_weighted_voting_collection,
    load_effective_settings,
    persist_authoritative_artifact,
    persist_effective_settings,
    snapshot_collection_for_key,
)
from backend.app.algorithms.weighted_voting.risk_budget import (
    WEIGHTED_VOTING_RISK_BUDGET_NAMESPACE,
    WEIGHTED_VOTING_RISK_BUDGET_VERSION,
    WeightedVotingOpenPositionRisk,
    WeightedVotingRiskBudget,
    build_weighted_voting_risk_budget,
)
from backend.app.algorithms.weighted_voting.runtime_context import (
    ExchangeSessionState,
    ExecutionCostEstimate,
    FORBIDDEN_AUTHORITATIVE_EVALUATION_INPUTS,
    GlobalRiskCapacity,
    PaperAccountSnapshot,
    RUNTIME_CONTEXT_FIELD_NAMES,
    WEIGHTED_VOTING_RUNTIME_CONTEXT_VERSION,
    WeightedVotingDataQualityState,
    WeightedVotingEventRiskState,
    WeightedVotingExchangeSessionState,
    WeightedVotingExecutionCostEstimate,
    WeightedVotingGlobalRiskState,
    WeightedVotingReadOnlyAccountObservation,
    WeightedVotingRuntimeContext,
    WeightedVotingRuntimeContextBuilder,
    WeightedVotingRuntimeFieldSource,
    WeightedVotingStaticAccountPort,
    WeightedVotingStaticGlobalRiskPort,
    WeightedVotingStaticMarketDataPort,
    WeightedVotingUnavailableAccountPort,
    WeightedVotingUnavailableGlobalRiskPort,
    payload_contains_forbidden_authoritative_evaluation_inputs,
    runtime_context_status,
)
from backend.app.algorithms.weighted_voting.runtime_supervisor import (
    WEIGHTED_VOTING_RUNTIME_SUPERVISOR_VERSION,
    WeightedVotingBarEventWorker,
    WeightedVotingDecisionWorker,
    WeightedVotingExecutionWorker,
    WeightedVotingFinalisedBarEvent,
    WeightedVotingHeartbeatWorker,
    WeightedVotingRecoveryWorker,
    WeightedVotingReconciliationWorker,
    WeightedVotingRiskWorker,
    WeightedVotingRuntimeConfig,
    WeightedVotingRuntimeSupervisor,
    WeightedVotingDailyUpdateWorker,
    WeightedVotingPositionManager,
    get_weighted_voting_runtime_supervisor,
    publish_weighted_voting_finalised_bar_event,
    runtime_supervisor_status,
    weighted_voting_bar_event_idempotency_key,
    weighted_voting_decision_idempotency_key,
    weighted_voting_market_event_id,
    weighted_voting_order_intent_idempotency_key,
)
from backend.app.algorithms.weighted_voting.scheduler import (
    ACTIVE_WEIGHT_STATE_KEY,
    CANDIDATE_WEIGHT_PREFIX,
    OUTCOME_HISTORY_KEY,
    PUBLISHED_WEIGHT_PREFIX,
    TRANSACTION_COST_PERFORMANCE_PREFIX,
    UPDATE_AUDIT_PREFIX,
    UPDATE_RECORD_PREFIX,
    UPDATE_STATUS_KEY,
    WEIGHT_HISTORY_KEY,
    WEIGHTED_VOTING_AFTER_MARKET_UPDATE_EASTERN_MINUTES,
    WEIGHTED_VOTING_SCHEDULER_VERSION,
    WeightedVotingDailySchedulerConfig,
    WeightedVotingDailyWeightUpdateResult,
    activate_published_weight_for_session,
    rollback_to_previous_weight_version,
    run_after_market_daily_weight_update,
    scheduler_status,
)
from backend.app.algorithms.weighted_voting.service import WEIGHTED_VOTING_SERVICE_VERSION, WeightedVotingService
from backend.app.algorithms.weighted_voting.signal_engine import WEIGHTED_VOTING_SIGNAL_ENGINE_VERSION, evaluate_signals
from backend.app.algorithms.weighted_voting.weight_engine import (
    WEIGHTED_VOTING_WEIGHT_ENGINE_VERSION,
    WEIGHTED_VOTING_WEIGHT_UPDATE_RULES,
    weight_engine_status,
)

__all__ = [
    "WEIGHTED_VOTING_ACTIVE_WEIGHT_VERSION",
    "ACTIVE_WEIGHT_STATE_KEY",
    "CANDIDATE_WEIGHT_PREFIX",
    "ExchangeSessionState",
    "ExecutionCostEstimate",
    "FORBIDDEN_AUTHORITATIVE_EVALUATION_INPUTS",
    "GlobalRiskCapacity",
    "PaperAccountSnapshot",
    "OUTCOME_HISTORY_KEY",
    "PUBLISHED_WEIGHT_PREFIX",
    "TRANSACTION_COST_PERFORMANCE_PREFIX",
    "UPDATE_AUDIT_PREFIX",
    "UPDATE_RECORD_PREFIX",
    "UPDATE_STATUS_KEY",
    "WEIGHT_HISTORY_KEY",
    "RUNTIME_CONTEXT_FIELD_NAMES",
    "WEIGHTED_VOTING_ALGORITHM_ID",
    "WEIGHTED_VOTING_ARCHITECTURE_VERSION",
    "WEIGHTED_VOTING_AUTHORITATIVE_PACKAGE",
    "WEIGHTED_VOTING_BACKTEST_KERNEL",
    "WEIGHTED_VOTING_ARCHITECTURE_CAPITAL_PARTITION_ID",
    "WEIGHTED_VOTING_DECISION_KERNEL",
    "WEIGHTED_VOTING_DECISION_KERNEL_VERSION",
    "WEIGHTED_VOTING_FILESYSTEM_ROOT",
    "WEIGHTED_VOTING_OWNED_MUTABLE_DOMAINS",
    "WEIGHTED_VOTING_PIPELINE_BOUNDARIES",
    "WEIGHTED_VOTING_SHARED_READ_ONLY_PORTS",
    "WEIGHTED_VOTING_STORAGE_NAMESPACE",
    "WEIGHTED_VOTING_API_NAMESPACE",
    "WEIGHTED_VOTING_API_TAG",
    "WEIGHTED_VOTING_API_VERSION",
    "WEIGHTED_VOTING_BASELINE_STRATEGY_WEIGHT",
    "WEIGHTED_VOTING_AFTER_MARKET_UPDATE_EASTERN_MINUTES",
    "WEIGHTED_VOTING_BASELINE_CONFIGURATION_KEY",
    "WEIGHTED_VOTING_CONFIG_VERSION",
    "WEIGHTED_VOTING_CONFIGURATION_NAMESPACE",
    "WEIGHTED_VOTING_CONFIGURATION_VERSION",
    "WEIGHTED_VOTING_ERROR_CODE_PREFIX",
    "WEIGHTED_VOTING_ENTRY_POLICY_FRONTEND_MODE",
    "WEIGHTED_VOTING_ENTRY_POLICY_VERSION",
    "WEIGHTED_VOTING_MAXIMUM_STRATEGY_WEIGHT",
    "WEIGHTED_VOTING_BROKER_CONNECTION_BOUNDARY",
    "WEIGHTED_VOTING_BROKER_RECONCILIATION_NAMESPACE",
    "WEIGHTED_VOTING_BROKER_RECONCILIATION_VERSION",
    "WEIGHTED_VOTING_EXECUTION_GATEWAY_VERSION",
    "WEIGHTED_VOTING_EXECUTION_NAMESPACE",
    "WEIGHTED_VOTING_EXECUTION_OWNERSHIP",
    "WEIGHTED_VOTING_EXIT_POLICY_VERSION",
    "WEIGHTED_VOTING_FINAL_ORDER_ACCEPTANCE_VERSION",
    "WEIGHTED_VOTING_ALLOWED_GLOBAL_ACTIONS",
    "WEIGHTED_VOTING_CAPITAL_PARTITION_ID",
    "WEIGHTED_VOTING_GLOBAL_RISK_CONTRACT_VERSION",
    "WEIGHTED_VOTING_GLOBAL_IMMUTABILITY_CHECKS",
    "WEIGHTED_VOTING_GLOBAL_INTERFACE_VERSION",
    "WeightedVotingGlobalRiskRequest",
    "WeightedVotingGlobalRiskResponse",
    "WeightedVotingStaticCentralGlobalRiskService",
    "WeightedVotingUnavailableCentralGlobalRiskService",
    "WEIGHTED_VOTING_INVENTORY_NAMESPACE",
    "WEIGHTED_VOTING_INVENTORY_VERSION",
    "WEIGHTED_VOTING_MARKET_SNAPSHOT_ADAPTER_VERSION",
    "WEIGHTED_VOTING_MINIMUM_STRATEGY_WEIGHT",
    "WEIGHTED_VOTING_MODULE_INVENTORY",
    "WEIGHTED_VOTING_ORDER_PROPOSAL_OWNERSHIP",
    "WEIGHTED_VOTING_ORDER_PROPOSAL_VERSION",
    "WEIGHTED_VOTING_PERFORMANCE_TRACKER_NAMESPACE",
    "WEIGHTED_VOTING_PERFORMANCE_TRACKER_VERSION",
    "WEIGHTED_VOTING_ARTIFACT_CATEGORIES",
    "WEIGHTED_VOTING_COLLECTION_ALIASES",
    "WEIGHTED_VOTING_PERSISTENCE_VERSION",
    "WEIGHTED_VOTING_POSITION_TRADE_STATE_NAMESPACE",
    "WEIGHTED_VOTING_POSITION_TRADE_STATE_VERSION",
    "WEIGHTED_VOTING_POSITION_MANAGER_NAMESPACE",
    "WEIGHTED_VOTING_POSITION_MANAGER_VERSION",
    "WEIGHTED_VOTING_REASON_CODE_PREFIX",
    "WEIGHTED_VOTING_REQUIRED_COLLECTIONS",
    "WEIGHTED_VOTING_REQUIRED_RECORD_FIELDS",
    "WEIGHTED_VOTING_RISK_BUDGET_NAMESPACE",
    "WEIGHTED_VOTING_RISK_BUDGET_VERSION",
    "WEIGHTED_VOTING_RUNTIME_CONTEXT_VERSION",
    "WEIGHTED_VOTING_SETTINGS_KEY",
    "WEIGHTED_VOTING_SERVICE_VERSION",
    "WEIGHTED_VOTING_SCHEDULER_VERSION",
    "WEIGHTED_VOTING_SIGNAL_ENGINE_VERSION",
    "WEIGHTED_VOTING_STORAGE_ROOT",
    "WEIGHTED_VOTING_WEIGHT_ENGINE_VERSION",
    "WEIGHTED_VOTING_WEIGHT_UPDATE_RULES",
    "WEIGHTED_VOTING_STRATEGY_VERSION",
    "WeightedVotingConfig",
    "WeightedVotingArchitectureContract",
    "WeightedVotingDailySchedulerConfig",
    "WeightedVotingDailyWeightUpdateResult",
    "activate_published_weight_for_session",
    "WeightedVotingDecisionKernel",
    "WeightedVotingDecisionResult",
    "WeightedVotingDedicatedStrategyInventoryItem",
    "WeightedVotingModuleInventory",
    "WeightedVotingModuleLifecycleStatus",
    "WeightedVotingModuleStatus",
    "WeightedEntryPolicyResult",
    "WeightedEntryTiming",
    "WeightedEntryType",
    "WeightedExitAction",
    "WeightedVotingAuthoritativeTradeRecord",
    "WeightedVotingBrokerCommand",
    "WeightedVotingBrokerFillObservation",
    "WeightedVotingBrokerOrderObservation",
    "WeightedVotingBrokerPositionObservation",
    "WeightedVotingBrokerReconciliationDiscrepancy",
    "WeightedVotingBrokerReconciliationResult",
    "WeightedVotingExecutionLifecycleRecord",
    "WeightedVotingExecutionQueueItem",
    "WeightedVotingExecutionReconciliation",
    "WeightedVotingExecutionRejection",
    "WeightedVotingExecutionStatus",
    "WeightedVotingExecutionSubmission",
    "WeightedVotingExitDecision",
    "WeightedVotingExitInstruction",
    "WeightedVotingExitInputs",
    "WeightedVotingExitLifecycleState",
    "WeightedVotingFinalOrderAcceptanceCheck",
    "WeightedVotingFinalOrderAcceptanceInputs",
    "WeightedVotingFinalOrderAcceptanceResult",
    "WeightedVotingCapitalPartition",
    "WeightedVotingDailyLedger",
    "WeightedVotingFillAttribution",
    "WeightedVotingAlgorithmPerformance",
    "WeightedVotingArtifactMetadata",
    "WeightedVotingFilesystemStateStore",
    "WeightedVotingMarketConditionPerformance",
    "WeightedVotingManagedOrderType",
    "WeightedVotingOpenPositionRisk",
    "WeightedVotingOrderLifecycle",
    "WeightedVotingOrderProposal",
    "WeightedVotingOrderState",
    "WeightedVotingInventoryEvent",
    "WeightedVotingInventoryEventType",
    "WeightedVotingInventoryRepository",
    "WeightedVotingInventorySnapshot",
    "WeightedVotingPendingOrder",
    "WeightedVotingPerformanceBucket",
    "WeightedVotingPerformanceReport",
    "WeightedVotingPerformanceSnapshot",
    "WeightedVotingPipelineBoundary",
    "WeightedVotingPositionLifecycle",
    "WeightedVotingPositionAttribution",
    "WeightedVotingPositionTradeState",
    "WeightedVotingPosition",
    "WeightedVotingPositionManagerService",
    "WeightedVotingProtectiveInstruction",
    "WeightedVotingRiskBudget",
    "WeightedVotingRiskUsage",
    "PaperAccountSnapshot",
    "WeightedVotingRuntimeContext",
    "WeightedVotingRuntimeContextBuilder",
    "WeightedVotingRuntimeFieldSource",
    "WEIGHTED_VOTING_RUNTIME_SUPERVISOR_VERSION",
    "WEIGHTED_VOTING_ALPACA_PAPER_BROKER_VERSION",
    "WEIGHTED_VOTING_LOCAL_PAPER_BROKER_VERSION",
    "WEIGHTED_VOTING_LOCAL_PAPER_NAMESPACE",
    "WeightedVotingAlpacaPaperBroker",
    "WeightedVotingAlpacaPaperBrokerConfigurationError",
    "WeightedVotingLocalPaperAccount",
    "WeightedVotingLocalPaperBroker",
    "WeightedVotingLocalPaperRiskPort",
    "WeightedVotingLocalPaperRiskService",
    "WeightedVotingUnavailablePaperBroker",
    "WeightedVotingBarEventWorker",
    "WeightedVotingDecisionWorker",
    "WeightedVotingExecutionWorker",
    "WeightedVotingFinalisedBarEvent",
    "WeightedVotingHeartbeatWorker",
    "WeightedVotingRecoveryWorker",
    "WeightedVotingReconciliationWorker",
    "WeightedVotingReconciliationDiscrepancySeverity",
    "WeightedVotingRiskWorker",
    "WeightedVotingRuntimeConfig",
    "WeightedVotingRuntimeSupervisor",
    "WeightedVotingDailyUpdateWorker",
    "WeightedVotingPositionManager",
    "WeightedVotingDataQualityState",
    "WeightedVotingEventRiskState",
    "WeightedVotingExchangeSessionState",
    "WeightedVotingExecutionCostEstimate",
    "WeightedVotingGlobalRiskState",
    "WeightedVotingReadOnlyAccountObservation",
    "WeightedVotingStaticAccountPort",
    "WeightedVotingStaticGlobalRiskPort",
    "WeightedVotingStaticMarketDataPort",
    "WeightedVotingUnavailableAccountPort",
    "WeightedVotingUnavailableGlobalRiskPort",
    "WeightedVotingReadOnlyPort",
    "WeightedVotingService",
    "WeightedVotingServiceBoundary",
    "WeightedVotingSignalObservation",
    "WeightedVotingStrategyPerformance",
    "WeightedVotingTrackedTrade",
    "WeightedVotingWeightVersionPerformance",
    "WeightedVotingWeightVersionSnapshot",
    "build_global_order_proposal_from_weighted_voting_proposal",
    "build_weighted_voting_broker_command",
    "build_weighted_voting_market_snapshot",
    "build_weighted_voting_global_risk_request",
    "build_weighted_voting_paper_gateway_dependencies",
    "build_weighted_voting_local_paper_gateway_dependencies",
    "build_weighted_voting_order_proposal",
    "build_weighted_voting_performance_report",
    "build_weighted_voting_risk_budget",
    "canonical_weighted_voting_collection",
    "assert_weighted_voting_position_ownership",
    "assert_weighted_voting_position_manager_ownership",
    "close_weighted_voting_position",
    "create_weighted_voting_order_state",
    "deterministic_weighted_voting_client_order_id",
    "decision_kernel_status",
    "enqueue_weighted_voting_execution_order",
    "entry_policy_status",
    "evaluate_exit_lifecycle",
    "evaluate_entry_policy",
    "evaluate_weighted_voting_final_order_acceptance",
    "execution_gateway_status",
    "exit_policy_status",
    "global_interface_status",
    "global_gate_response_from_weighted_voting_risk",
    "validate_weighted_voting_global_risk_response",
    "inventory_status",
    "load_effective_settings",
    "open_exit_lifecycle",
    "mark_weighted_voting_order_cancelled",
    "mark_weighted_voting_order_filled",
    "mark_weighted_voting_order_open",
    "mark_weighted_voting_order_partial_fill",
    "mark_weighted_voting_order_rejected",
    "open_weighted_voting_position_from_order",
    "persist_weighted_voting_broker_command",
    "persist_authoritative_artifact",
    "persist_effective_settings",
    "persist_weighted_voting_order_state",
    "persist_weighted_voting_performance_report",
    "persist_weighted_voting_position_state",
    "performance_tracker_status",
    "position_trade_state_status",
    "position_manager_status",
    "runtime_context_status",
    "runtime_supervisor_status",
    "payload_contains_forbidden_authoritative_evaluation_inputs",
    "snapshot_collection_for_key",
    "reconcile_weighted_voting_broker_result",
    "reconcile_weighted_voting_broker_observations",
    "reconciliation_status",
    "record_weighted_voting_fill",
    "record_weighted_voting_rejection",
    "record_weighted_voting_submission",
    "rollback_to_previous_weight_version",
    "run_after_market_daily_weight_update",
    "scheduler_status",
    "get_weighted_voting_runtime_supervisor",
    "publish_weighted_voting_finalised_bar_event",
    "weighted_voting_bar_event_idempotency_key",
    "weighted_voting_decision_idempotency_key",
    "weighted_voting_market_event_id",
    "weighted_voting_order_intent_idempotency_key",
    "submit_queued_weighted_voting_paper_order",
    "submit_weighted_voting_paper_order",
    "update_weighted_voting_position_mark",
    "evaluate_signals",
    "weighted_voting_dedicated_strategy_inventory",
    "weighted_voting_enabled_strategy_catalog",
    "weighted_voting_architecture_contract",
    "weighted_voting_architecture_status",
    "weighted_voting_module_inventory",
    "weighted_voting_reason_code",
    "weighted_voting_service_boundary",
    "weight_engine_status",
]
