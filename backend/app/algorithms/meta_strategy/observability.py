...
def apply_meta_strategy_operational_control(
    *,
    ...
    if control == "cancel_pending_jobs":
    ...
    elif control == "force_reconciliation":
        job = job_repository.enqueue_job(
            job_type="inventory_reconciliation",
            idempotency_key=f"meta_strategy.force_reconciliation.{body.get('idempotencyKey') or current.isoformat()}",
            payload={"source": "operational_control", "actor": actor, "reason": reason},
            now=current,
        )
        result_payload["jobId"] = job.job_id
    elif control == "model_rollback":
        result_payload.update(job_repository.rollback_active_model(actor=actor, reason=reason, now=current))
    else:
        result_payload["state"] = _control_state(control, body)
    snapshot_key = f"{META_STRATEGY_CONTROL_SNAPSHOT_PREFIX}{control}"
    job_repository.write_gateway_snapshot(snapshot_key, result_payload, now=current)
    evidence = job_repository.record_operational_event(f"control.{control}", result_payload, status=status, correlation_id=str(body.get("correlationId") or ""), now=current)
    return MetaStrategyOperationalControlResult(
        ...
        reason_codes=(f"meta_strategy.controls.{control}.recorded",),
    )
