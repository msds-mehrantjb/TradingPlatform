...
class VotingEnsembleLocalGateEngine:
    ...
    def _risk_limits(self, context: GlobalGateInput) -> list[GateCheckResult]:
        ...
        # Documented inert: nothing sets existingPositionConflict on the account snapshot, so
        # this never fails. What would feed it is a producer flag comparing the candidate's
        # direction with the open local position; left unset on purpose, since an opposite
